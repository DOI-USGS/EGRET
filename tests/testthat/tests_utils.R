...
test_that("date functions work", {
  ...
  expect_warning(checkStartEndDate(
    "2017-01-01",
    "2016-01-01",
    interactive = FALSE
  ))
...
test_that("data functions work", {
  ...
  expect_true(all(
    c("dateTime", "ConcLow", "ConcHigh", "Uncen") %in%
      names(compressed)
  ))
...
test_that("other plot functions don't error", {
  ...
  graphics.off()
  dev_start <- dev.cur()
  expect_silent(plotFluxQ(eList, fluxUnit = 4, qUnit = 1))
  expect_true(dev_start + 1 == dev.cur())
  ...
  yearEnd <- 2009
  yearStart <- 2000
  expect_silent(plotConcTimeSmooth(
    eList,
    q1,
    q2,
    q3,
  ...
  graphics.off()
  dev_start <- dev.cur()
  expect_silent(fluxBiasMulti(eList, qUnit = 1, fluxUnit = 4))
  expect_true(dev_start + 1 == dev.cur())
...
test_that("flexPlotAddOn functions properly", {
  ...
  expect_true(all(is.na(blank_daily$FNConc[
    blank_daily$Date > startBlank &
      blank_daily$Date < endBlank
  ])))

  not_blank <- getDaily(eList)
  expect_false(all(is.na(not_blank$FNConc[
    not_blank$Date > startBlank &
...
test_that("mergeReport", {
  ...
  expect_true(all(
    names(eList_orig_Ch) %in% c("INFO", "Daily", "Sample", "surfaces")
  ))

  expect_equal(
    round(head(eList_orig_Ch$Sample$Q), 3),
