...
test_that("modelEstimation produces correct values with default args", {
  ...
  expect_equal(
    sapply(info_orig_Ch, '[[', 1),
    sapply(info_modeled[, orig_cols_info], '[[', 1)
  )
  ...
  sample_modeled_reordered <- sample_modeled[, match(
    orig_colnames,
    model_colnames
  )]
  expect_equal(
    sapply(sample_orig_Ch, '[[', 1),
    sapply(sample_modeled_reordered, '[[', 1)
  )
  ...
  expect_true(all(
    names(eList_modeled) %in% c("INFO", "Daily", "Sample", "surfaces")
  ))
...
test_that('setUpEstimation handles missing info well', {
  ...
  expect_error(
    setUpEstimation(eList_zero),
    "modelEstimation cannot be run with 0 values in ConcLow."
  )
