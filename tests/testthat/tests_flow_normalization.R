...
test_that("setupYears", {
  ...
  expect_equal(
    signif(AnnualResults_calendarYear$DecYear[1], digits = 7),
    1980.5
  )
  expect_equal(
    signif(AnnualResults_calendarYear$FNConc[1], digits = 7),
    0.9994875
  )
  expect_equal(
    signif(AnnualResults_calendarYear$FNFlux[1], digits = 7),
    292.0164
  )
  expect_equal(
    signif(AnnualResults_calendarYear$Conc[1], digits = 7),
    0.9979275
  )
...
test_that("setupYears", {
  ...
  tFC <- tableFlowChange(
    eList,
    istat = 5,
    yearPoints = c(1985, 1990, 1995, 2001, 2005, 2009)
  )
  expect_true(all(
    names(tFC) %in%
      c(
  ...
  expect_equal(
    setSeasonLabelByUser(paStartInput = 12, paLongInput = 3),
...
test_that("getSurfaceEstimates", {
  ...
  expect_true(all(
    c("yHat", "SE", "ConcDay", "FluxDay") %in% names(daily_stuff)
  ))
  expect_equal(
    round(head(daily_stuff$yHat), 3),
    c(-0.165, -0.180, -0.304, -0.464, -0.487, -0.368)
  )
  expect_equal(
    round(head(daily_stuff$SE), 3),
    c(0.221, 0.221, 0.217, 0.212, 0.211, 0.215)
  )
  expect_equal(
    round(head(daily_stuff$ConcDay), 3),
    c(0.875, 0.860, 0.764, 0.654, 0.638, 0.721)
  )
  expect_equal(
    round(head(daily_stuff$FluxDay), 3),
...
test_that("bin_Qs", {
  ...
  expect_equal(
    round(head(allLogQsByDayOfYear[[1]]), 3),
    c(1.041, 0.425, 1.118, 0.830, 1.773, -0.009)
  )
})

test_that("dateInfo", {
  dateInfoA <- makeDateInfo(
    windowSide = 7,
...
test_that("getConcFluxFromSurface", {
  ...
  concFlux_list <- EGRET:::getConcFluxFromSurface(
    eList,
    allLogQsByDayOfYear,
    localDaily
  )
  ...
  expect_equal(
    round(head(concFlux_list[["allFluxReplicated"]]), 3),
    c(143.356, 100.241, 88.592, 47.434, 88.592, 92.699)
  )
  expect_equal(
    round(head(concFlux_list[["allConcReplicated"]]), 3),
    c(0.875, 1.024, 1.065, 1.212, 1.065, 1.052)
  )
  expect_equal(
    round(head(concFlux_list[["allDatesReplicated"]]), 3),
...
test_that("runPairs", {
  ...
  expect_true(all(
    names(pairOut_1) %in%
      c("TotalChange", "CQTC", "QTC", "x10", "x11", "x20", "x22")
  ))
  ...
  expect_equal(
    round(
      attr(pairOut_1, "Other")[["PercentChangeConc"]][["Total Percent Change"]],
      digits = 2
    ),
    42.27
  )
  expect_equal(
    round(
      attr(pairOut_1, "Other")[["PercentChangeConc"]][["CQTC Percent"]],
      digits = 2
    ),
    42.27
  )
  expect_equal(
    round(
      attr(pairOut_1, "Other")[["PercentChangeConc"]][["QTC Percent"]],
      digits = 2
    ),
    0
  )
  expect_equal(
    round(
      attr(pairOut_1, "Other")[["PercentChangeFlux"]][["Total Percent Change"]],
      digits = 2
    ),
    29.44
  )
  expect_equal(
    round(
      attr(pairOut_1, "Other")[["PercentChangeFlux"]][["CQTC Percent"]],
      digits = 2
    ),
    29.44
  )
  expect_equal(
    round(
      attr(pairOut_1, "Other")[["PercentChangeFlux"]][["QTC Percent"]],
      digits = 2
    ),
    0
  )
  ...
  expect_equal(
    round(
      attr(pairOut_2, "Other")[["PercentChangeConc"]][["Total Percent Change"]],
      digits = 2
    ),
    38.56
  )
  expect_equal(
    round(
      attr(pairOut_2, "Other")[["PercentChangeConc"]][["CQTC Percent"]],
      digits = 2
    ),
    41.7
  )
  expect_equal(
    round(
      attr(pairOut_2, "Other")[["PercentChangeConc"]][["QTC Percent"]],
      digits = 2
    ),
    -3.13
  )
  expect_equal(
    round(
      attr(pairOut_2, "Other")[["PercentChangeFlux"]][["Total Percent Change"]],
      digits = 2
    ),
    50.68
  )
  expect_equal(
    round(
      attr(pairOut_2, "Other")[["PercentChangeFlux"]][["CQTC Percent"]],
      digits = 2
    ),
    31.94
  )
  expect_equal(
    round(
      attr(pairOut_2, "Other")[["PercentChangeFlux"]][["QTC Percent"]],
      digits = 2
    ),
    18.74
  )
  ...
  expect_equal(
    round(
      attr(pairOut_3, "Other")[["PercentChangeConc"]][["Total Percent Change"]],
      digits = 2
    ),
    39.77
  )
  expect_equal(
    round(
      attr(pairOut_3, "Other")[["PercentChangeConc"]][["CQTC Percent"]],
      digits = 2
    ),
    41.81
  )
  expect_equal(
    round(
      attr(pairOut_3, "Other")[["PercentChangeConc"]][["QTC Percent"]],
      digits = 2
    ),
    -2.04
  )
  expect_equal(
    round(
      attr(pairOut_3, "Other")[["PercentChangeFlux"]][["Total Percent Change"]],
      digits = 2
    ),
    45.81
  )
  expect_equal(
    round(
      attr(pairOut_3, "Other")[["PercentChangeFlux"]][["CQTC Percent"]],
      digits = 2
    ),
    31.89
  )
  expect_equal(
    round(
      attr(pairOut_3, "Other")[["PercentChangeFlux"]][["QTC Percent"]],
      digits = 2
    ),
    13.91
  )
  ...
  expect_equal(
    round(
      attr(pairOut_4, "Other")[["PercentChangeConc"]][["Total Percent Change"]],
      digits = 2
    ),
    38.93
  )
  expect_equal(
    round(
      attr(pairOut_4, "Other")[["PercentChangeConc"]][["CQTC Percent"]],
      digits = 2
    ),
    41.81
  )
  expect_equal(
    round(
      attr(pairOut_4, "Other")[["PercentChangeConc"]][["QTC Percent"]],
      digits = 2
    ),
    -2.88
  )
  expect_equal(
    round(
      attr(pairOut_4, "Other")[["PercentChangeFlux"]][["Total Percent Change"]],
      digits = 2
    ),
    50.44
  )
  expect_equal(
    round(
      attr(pairOut_4, "Other")[["PercentChangeFlux"]][["CQTC Percent"]],
      digits = 2
    ),
    31.89
  )
  expect_equal(
    round(
      attr(pairOut_4, "Other")[["PercentChangeFlux"]][["QTC Percent"]],
      digits = 2
    ),
    18.55
  )
...
test_that("runSeries", {
  ...
  expect_equal(
    round(attr(seriesOut_1$surfaces, "Year")[1], digits = 2),
    1979.69
  )
  ...
  expect_equal(
    round(attr(seriesOut_3$surfaces, "Year")[1], digits = 2),
    1979.69
  )

  # Option 4:
  seriesOut_4 <- runSeries(
    eList,
    windowSide = 7,
    flowBreak = TRUE,
    Q1EndDate = "1990-09-30"
  ...
  expect_equal(
    round(attr(seriesOut_4$surfaces, "Year")[1], digits = 2),
    1979.69
  )
})
...
test_that("runGroups", {
  ...
  groupOut_1 <- runGroups(
    eList,
    windowSide = 0,
    group1firstYear = 1980,
    group1lastYear = 1990,
    group2firstYear = 1995,
    group2lastYear = 2005
  )
  ...
  expect_true(all(
    round(groupOut_1$TotalChange, digits = 3) %in% c(0.226, 0.022)
  ))
  ...
  expect_true(all(
    round(groupOut_2$TotalChange, digits = 3) %in% c(0.200, 0.039)
  ))
  ...
  expect_true(all(
    round(groupOut_3$TotalChange, digits = 3) %in% c(0.207, 0.037)
  ))
  ...
  expect_true(all(
    round(groupOut_4$TotalChange, digits = 3) %in% c(0.203, 0.040)
  ))
...
test_that("stitch", {
  ...
  expect_true(all(
    names(attributes(surface_skip)) %in%
      c(
        "dim",
        "surfaceIndex",
        "Year",
