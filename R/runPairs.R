...
runPairs <- function(
  ...
  if (wall & oldSurface) {
    message(
      "Setting both arguments wall and oldSurfaces to TRUE are not allowed."
    )
  ...
  if (windowSide <= 0 && !flowBreak) {
    flowStart <- c(
      startEndSurface1[["startDate"]],
      startEndSurface2[["startDate"]]
    )
  ...
  Sample1 <- localSample[
    localSample$Date >= sample1StartDate &
      localSample$Date <= sample1EndDate,
  ]
  Sample2 <- localSample[
    localSample$Date >= sample2StartDate &
      localSample$Date <= sample2EndDate,
  ]

  fractMin <- min(fractMin, 1.0)

  minNumObs <- ceiling(min(
    minNumObs,
  ...
  message(
    "minNumObs has been set to ",
    minNumObs,
    " minNumUncen has been set to ",
    minNumUncen
  )
  check <- rep(1, 4)
  ...
  Daily1 <- localDaily[
    localDaily$Date >= dateInfo$flowNormStart[1] &
      localDaily$Date <= dateInfo$flowNormEnd[1],
  ]
  Daily2 <- localDaily[
    localDaily$Date >= dateInfo$flowNormStart[2] &
  ...
  if (oldSurface) {
    ...
    if (
      all(
        c("Year", "LogQ", "surfaceIndex") %in% names(attributes(localsurfaces))
      )
    ) {
    ...
    surfIndex1 <- c(
      surfIndex1[1] - 1,
      surfIndex1,
      surfIndex1[length(surfIndex1)] + 1
    )
    surfIndex2 <- which(surfaceYear >= startDec2 & surfaceYear <= endDec2)
    surfIndex2 <- c(
      surfIndex2[1] - 1,
      surfIndex2,
      surfIndex2[length(surfIndex2)] + 1
    )
  ...
  DailyRS1FD1 <- estDailyFromSurfaces(
    eList,
  ...
  if (is.nan(c11)) {
    stop(
      "year1 does not have adaquate Daily data (missing >10%).
         Adjusting year1."
    )
  }
  DailyRS2FD2 <- estDailyFromSurfaces(
    eList,
  ...
  if (is.nan(c22)) {
    stop(
      "year2 does not have adaquate Daily data (missing >10%).
         Adjusting to year2."
    )
  }
  Daily0 <- localDaily[
    localDaily$Date >= QStartDate & localDaily$Date <= QEndDate,
  ]
  DailyRS1FD0 <- estDailyFromSurfaces(
    eList,
    localsurfaces = surfaces1,
    localDaily = Daily0
  )
  ...
  DailyRS2FD0 <- estDailyFromSurfaces(
    eList,
    localsurfaces = surfaces2,
    localDaily = Daily0
  )
  ...
  pairResults[2, ] <- 0.00036525 *
    c(fDeltaTotal, fRSpart, fFDpart, f10, f11, f20, f22)
  ...
  Other <- list(
    ...
    PercentChangeConc = c(
      "Total Percent Change" = totChangePct_conc,
      "CQTC Percent" = CQTC_percent_conc,
      "QTC Percent" = QTC_percent_conc
    ),
    PercentChangeFlux = c(
      "Total Percent Change" = totChangePct_flux,
  ...
  monthlyResults1$monthFlux <- monthlyResults1$nDays *
    monthlyResults1$FNFlux /
    eList$INFO$drainSqKm
  monthlyResults1$monthConc <- monthlyResults1$FNConc

  monthlyResults2$monthFlux <- monthlyResults2$nDays *
    monthlyResults2$FNFlux /
    eList$INFO$drainSqKm
...
printPairs <- function(eList, pairResults) {
  ...
  totChangePct_conc_f <- add_plus(Other$PercentChangeConc[[
  ...
  cat(
    "\n\n Concentration v. Q Trend Component ",
    pctRS,
    "\n       Q Trend Component            ",
    pctFD,
    " \n\n"
  ...
  totChangePct_flux_f <- add_plus(Other$PercentChangeFlux[[
    "Total Percent Change"
  ]])
