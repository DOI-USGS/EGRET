...
stitch <- function(
  ...
  message(
    "\n Sample1 mean concentration ",
    round(mean(Sample1$ConcAve), digits = 3)
  )
  message(
    "\n Sample2 mean concentration ",
    round(mean(Sample2$ConcAve), digits = 3)
  )
  ...
  DecLow <- highLow[["DecLow"]]
  sliceIndex <- which(
    vectorYear >= decimalDate(surface2Start) &
  ...
  if (!(any(Year1 %in% Year2) | any(Year2 %in% Year1))) {
    surfaceTotal <- array(
      c(surfaces1, surfaces2),
      dim = c(14, length(Year1) + length(Year2), 3)
    )
