...
plotQTimeDaily <- function(
  ...
  yearStart <- if (is.na(yearStart)) {
    as.integer(min(localDaily$DecYear, na.rm = TRUE))
  } else {
    yearStart
  }
  yearEnd <- if (is.na(yearEnd)) {
    as.integer(max(localDaily$DecYear, na.rm = TRUE))
  } else {
    yearEnd
  }
