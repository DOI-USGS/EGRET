...
#' expandedDateDF <- populateDateColumns(dateTime)
#' expandedDateDF
populateDateColumns <- function(rawData) {
  # rawData is a vector of dates
  ...
  DateFrame$Julian <- as.numeric(julian(
    dateTime,
  ...
  DateFrame$Day[which(DateFrame$Day > 59)] <- DateFrame$Day[which(
    DateFrame$Day > 59
  )] +
    leapOffset[which(DateFrame$Day > 59)]
  ...
  DateFrame$waterYear[DateFrame$Month %in% c(10:12)] <- DateFrame$waterYear[
    DateFrame$Month %in% c(10:12)
  ] +
    1
