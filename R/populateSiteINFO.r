...
populateSiteINFO <- function(INFO, siteNumber,interactive=TRUE){
  if (nzchar(siteNumber)){
    ...
    if (interactive){
      cat("Your site for streamflow data is", as.character(INFO$site_no),".\n")
      if (!nzchar(INFO$station_nm)){
        cat("No station name was listed in the USGS site file for site: ", INFO$site_no, ". Please enter a station name here(no quotes): \n")
      ...
      cat("The drainage area at this site is ", INFO$drain_area_va, "square miles which is being stored as", INFO$drainSqKm, "square kilometers.\n")    
    } else {
      INFO$drain_area_va <- as.numeric(INFO$drain_area_va)
      INFO$contrib_drain_area_va <- as.numeric(INFO$contrib_drain_area_va)
      INFO$drainSqKm <- INFO$drain_area_va * 2.5899881      
