...
setupYears <- function(localDaily, paLong = 12, paStart = 10) {
  ...
  complete <- numDays - 1 ==
    as.numeric(
      max(localDaily$Date, na.rm = TRUE) -
  ...
  withinIndex <- which(
    (StartEndSeq$Starts >= firstMonthSeq) & (StartEndSeq$Ends <= lastMonthSeq)
  )
  ...
  for (i in 1:numYears) {
    ...
    DailyYear <- localDaily[
      which(localDaily$MonthSeq %in% startMonth:stopMonth),
    ]
