...
plotContours <- function(
  ...
  cex.axis = 1,
  concLab = 1,
  color.palette = grDevices::colorRampPalette(c(
    "white",
    "gray",
    "blue",
    "red"
  ...
  qBottomT <- ifelse(
    is.na(qBottom),
    stats::quantile(localDaily$Q, probs = 0.05) * qFactor,
    qBottom
  )

  qTopT <- ifelse(
    is.na(qTop),
    stats::quantile(localDaily$Q, probs = 0.95) * qFactor,
    qTop
  )
  ...
  v1 <- if (is.na(vert1)) {
    ...
    c(vert1, log(yTicks[1], 10), vert1, log(yTicks[nYTicks], 10))
  }
  v2 <- if (is.na(vert2)) {
    vectorNone
  } else {
    c(vert2, log(yTicks[1], 10), vert2, log(yTicks[nYTicks], 10))
  }
  h1 <- if (is.na(horiz)) {
    vectorNone
  } else {
    c(yearStart, log(horiz, 10), yearEnd, log(horiz, 10))
  }
  ...
  graphics::filled.contour(
    ...
    plot.axes = {
      ...
      segments(
        xTicks,
        rep(log(yTicks[1], 10), length(xTicks)),
        xTicks,
        rep(
          graphics::grconvertY(
            graphics::grconvertY(par("usr")[3], from = "user", to = "inches") +
      ...
      segments(
        xTicks,
        rep(log(yTicks[nYTicks], 10), length(xTicks)),
        xTicks,
        rep(
          graphics::grconvertY(
            graphics::grconvertY(par("usr")[4], from = "user", to = "inches") -
              tcl,
            from = "inches",
            to = "user"
          ),
      ...
      segments(
        rep(
          graphics::grconvertX(
            graphics::grconvertX(par("usr")[2], from = "user", to = "inches") -
              tcl,
            from = "inches",
            to = "user"
          ),
