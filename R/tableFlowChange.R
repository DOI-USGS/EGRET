...
tableFlowChange <- function(
  ...
  cat("\n  ", localINFO$shortName)
  periodName <- setSeasonLabelByUser(
  ...
  if (runoff) {
    ...
    formatSpacing <- if (3 == nchar(gsub(" ", "", unitsText))) {
      "       "
    } else {
      "   "
    }
  ...
  header <- c(
    "year1",
    "year2",
    paste("change[", gsub(" ", "", unitsText), "]", sep = ""),
    paste("slope[", gsub(" ", "", unitsText), "/yr]", sep = ""),
    "change[%]",
    "slope[%/yr]"
  )
  ...
  for (iFirst in 1:numPointsMinusOne) {
    ...
    for (iLast in iFirstPlusOne:numPoints) {
      ...
      results[1] <- if (is.na(yDif)) {
        ...
        format(yDif, digits = 2, width = 12)
      }
      results[2] <- if (is.na(yDif)) {
        blankHolder
      } else {
        format(yDif / xDif, digits = 2, width = 12)
      }
      results[3] <- if (is.na(yDif)) {
        blankHolder
      } else {
        format(100 * yDif / yFirst, digits = 2, width = 12)
      }
      results[4] <- if (is.na(yDif)) {
        blankHolder
      } else {
        format(100 * yDif / yFirst / xDif, digits = 2, width = 12)
      }
