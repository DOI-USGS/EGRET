...
plotConcTimeDaily <- function(
  ...
  yearStart <- if (is.na(yearStart)) {
    as.integer(min(localSample$DecYear, na.rm = TRUE))
  } else {
    yearStart
  }
  yearEnd <- if (is.na(yearEnd)) {
    as.integer(max(localSample$DecYear, na.rm = TRUE))
  } else {
    yearEnd
  }
  ...
  localSample <- if (paLong == 12) {
    localSample
  } else {
    selectDays(localSample, paLong, paStart)
  }
  localDaily <- if (paLong == 12) {
    localDaily
  } else {
    selectDays(localDaily, paLong, paStart)
  }
  ...
  subSample <- localSample[
    localSample$DecYear >= yearStart &
      localSample$DecYear <= yearEnd,
  ]
  subDaily <- localDaily[
    localDaily$DecYear >= yearStart &
  ...
  if (!randomCensored) {
    ...
    if (!("rObserved" %in% names(localSample))) {
      ...
      subSample <- localSample[
        localSample$DecYear >= yearStart & localSample$DecYear <= yearEnd,
      ]
    }
  ...
  lines(x = xDaily, y = concentration, type = "l", col = col, lwd = lwd)
