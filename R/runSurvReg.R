...
runSurvReg <- function(
  ...
  if (run.parallel) {
    `%dopar%` <- foreach::`%dopar%`
    wrtds_return_list <- foreach::foreach(
      n = 1:numEstPt,
      .packages = c('EGRET')
    ) %dopar%
      {
        wrtds_returns <- run_WRTDS(
          estY = estPtYear[n],
          estLQ = estPtLQ[n],
    ...
    warningFlag <- sum(sapply(wrtds_return_list, function(x) {
      x[["warningFlag"]]
    }))
  ...
  if (warningFlag > 0) {
    message(
      ...
      numEstPt,
      " times (for different combinations of discharge and time).  In ",
      warningFlag,
      " of these runs it did not properly converge. This does not mean that the model is unacceptable, but it is a suggestion that there may be something odd about the data set. You may want to check for outliers, repeated values on a single date, or something else unusual about the data."
    )
  }
...
run_WRTDS <- function(
  ...
  x <- tryCatch(
    {
      survModel <- survival::survreg(
        survival::Surv(log(ConcLow), log(ConcHigh), type = "interval2") ~
          DecYear + LogQ + SinDY + CosDY,
        data = Sam,
        weights = weight,
        dist = "gaus"
      )
