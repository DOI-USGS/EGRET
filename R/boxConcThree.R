...
boxConcThree <- function(
  ...
  plotTitle <- if (printTitle) {
    paste0(
      ...
      "s\nwith estimates on sampled days and on all days using ",
      moreTitle
    )
  } else {
    ""
  ...
  if (tinyPlot) {
    yLab <- paste0(concPrefix@shortPrefix, ". (", localINFO$param.units, ")")
