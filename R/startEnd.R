...
startEnd <- function(paStart, paLong, year) {
  ...
  endDate <- if (isLeap & endMonth == 2) {
    paste(as.character(year), "-02-29", sep = "")
  } else {
    endDate
  }
  startEnd <- list(startDate = as.Date(startDate), endDate = as.Date(endDate))
  return(startEnd)
}
...
surfaceStartEnd <- function(paStart, paLong, Date1, Date2) {
  ...
  return(list(
    surfaceStart = as.Date(startSurface),
