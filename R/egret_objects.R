...
as.egret <- function(INFO, Daily, Sample = NA, surfaces = NA) {
  ...
  if (exists("Daily") && !all(is.na(Daily))) {
    ...
    if (!all(expectedCols %in% names(Daily))) {
      message(
        "\nDaily data frame expecting columns: ",
        expectedCols[!expectedCols %in% names(Daily)]
      )
    }
  ...
  if (exists("Sample") && !all(is.na(Sample))) {
    if (any(duplicated(Sample$Date))) {
      message(
        "\nThere are ",
        sum(duplicated(Sample$Date)),
        " duplicated Sample dates."
      )
    }
    ...
    if (
      ...
      message(
        "\nMissing columns:",
        c("ConcLow", "ConcHigh", "Uncen", "ConcAve")[
          !(c("ConcLow", "ConcHigh", "Uncen", "ConcAve") %in% names(Sample))
        ]
      )
    }

    int_indexs <- which(
      names(Sample) %in% c("Julian", "Month", "Day", "MonthSeq", "waterYear")
    )
...
is.egret <- function(x) {
  ...
  if (!all(c("INFO", "Daily") %in% names(x))) {
    stop(
      "Missing ",
      c("INFO", "Daily")[!(c("INFO", "Daily") %in% names(x))],
      " data frame"
    )
  }
  return(all(c(all(c("INFO", "Daily") %in% names(x)), inherits(x, "egret"))))
}
...
print.egret <- function(x, ...) {
  ...
  if (!all(is.na(x$Sample))) {
    columnsToPrint <- which(
      names(localSample) %in% c("Date", "ConcLow", "ConcHigh", "Q")
    )
