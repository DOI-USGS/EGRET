...
plotFluxHist <- function(
  ...
  localAnnualResults <- localAnnualResults[
    localAnnualResults$DecYear >= yearStart &
      localAnnualResults$DecYear <= yearEnd,
  ]

  subAnnualResults <- localAnnualResults[
    localAnnualResults$DecYear >= yearStart &
      localAnnualResults$DecYear <= yearEnd,
  ]
  ...
  if (plotAnnual & plotGenFlux & plotFlowNorm) {
    ...
    # only flow-normalized
    title3 <- "\nFlow Normalized Flux Estimates"
  } else if (plotFlowNorm & (plotGenFlux | plotAnnual)) {
    # flow normalized with 1
  ...
  if (plotAnnual) {
    with(
      subAnnualResults,
      points(
        subAnnualResults$DecYear[
          DecYear >= xInfo$bottom & DecYear <= xInfo$top
        ],
  ...
  if (plotGenFlux) {
    points(
      subAnnualResults$DecYear[
        subAnnualResults$DecYear >= xInfo$bottom &
          subAnnualResults$DecYear <= xInfo$top
      ],
      unitFactorReturn *
        subAnnualResults$GenFlux[
          subAnnualResults$DecYear >= xInfo$bottom &
            subAnnualResults$DecYear <= xInfo$top
        ],
      col = col.gen,
