...
populateSampleColumns <- function(rawData) {
  # rawData is a dataframe with dateTime, ConcLow, ConcHigh, Uncen
  Sample <- rawData
  ...
  Sample$ConcAve <- ifelse(
    Sample$ConcHigh == 0,
    Sample$ConcLow * 1.5,
    (Sample$ConcLow + Sample$ConcHigh) / 2
  )
  Sample$ConcLow <- ifelse(
    (rawData$ConcLow == 0.0 & rawData$Uncen == 0),
    NA,
    rawData$ConcLow
  )
  Sample$ConcHigh <- ifelse(
    (rawData$ConcHigh == 0 & rawData$Uncen == 0),
    NA,
    rawData$ConcHigh
  )
