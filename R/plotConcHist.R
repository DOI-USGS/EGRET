...
plotConcHist <- function(
  ...
  } else if (plotFlowNorm & (plotGenConc | plotAnnual)) {
  ...
  combinedY <- c(localAnnualResults$FNConc[
    localAnnualResults$DecYear > xInfo$bottom &
  ...
  if (plotAnnual) {
    with(
      localAnnualResults,
      points(
        DecYear[DecYear >= xInfo$bottom & DecYear <= xInfo$top],
