...
plotFluxPred <- function(
  ...
  if (tinyPlot) {
    ...
    yLab <- substitute(
      a ~ b,
      list(a = "Obs.", b = fluxUnit@unitExpressTiny[[1]])
    )
  } else {
    if (usgsStyle) {
      ...
      xLab <- substitute(
        a ~ b,
        list(a = "Estimated", b = fluxUnit@unitEstimate[[1]])
      )
      yLab <- substitute(
        a ~ b,
        list(a = "Observed", b = fluxUnit@unitEstimate[[1]])
      )
    }
  }
