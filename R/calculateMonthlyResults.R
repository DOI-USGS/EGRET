...
calculateMonthlyResults <- function(eList) {
  ...
  nDays <- stats::aggregate(
    localDaily$ConcDay,
    by = list(localDaily$MonthSeq),
    function(x) sum(!is.na(x))
  )$x

  Q <- stats::aggregate(localDaily$Q, by = list(localDaily$MonthSeq), mean)$x
  DecYear <- stats::aggregate(
    localDaily$DecYear,
    by = list(localDaily$MonthSeq),
    mean
  )$x
  ...
  Conc <- stats::aggregate(
    localDaily$ConcDay,
    by = list(localDaily$MonthSeq),
    mean
  )$x
  Flux <- stats::aggregate(
    localDaily$FluxDay,
    by = list(localDaily$MonthSeq),
    mean
  )$x
  FNConc <- stats::aggregate(
    localDaily$FNConc,
    by = list(localDaily$MonthSeq),
    mean
  )$x
  FNFlux <- stats::aggregate(
    localDaily$FNFlux,
    by = list(localDaily$MonthSeq),
    mean
  )$x
  ...
  if (kalman) {
    GenConc <- stats::aggregate(
      localDaily$GenConc,
      by = list(localDaily$MonthSeq),
      mean
    )$x
    GenFlux <- stats::aggregate(
      localDaily$GenFlux,
      by = list(localDaily$MonthSeq),
      mean
    )$x

    MonthlyResults <- data.frame(
      Month,
      Year,
      nDays,
      DecYear,
