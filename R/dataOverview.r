...
dataOverview <- function(Daily, Sample) {
  ...
  cat(
    "\n First day of the discharge record is",
    as.character(Daily$Date[1]),
    "and last day is",
    as.character(Daily$Date[numDays])
  )
  cat("\n The water quality record has", numSamples, "samples")
  cat(
    "\n The first sample is from",
    as.character(Sample$Date[1]),
    "and the last sample is from",
    as.character(Sample$Date[numSamples])
  )
  if (Sample$Date[1] < Daily$Date[1]) {
    cat(
      "\n WE HAVE A PROBLEM first sample is from before the first daily discharge"
    )
  }
  if (Sample$Date[numSamples] > Daily$Date[numDays]) {
    cat(
  ...
  cat(
    "\n Percentage of the sample values that are censored is",
    signif(pct, digits = 2),
    "%"
  )
}
