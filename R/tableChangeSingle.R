...
tableChangeSingle <- function(
  ...
  if (!("ConcDay" %in% names(localDaily))) {
    stop(
      "This function is only appropriate after running modelEstimation. It requires a ConcDay column in the Daily dataframe."
    )
  }
  ...
  if (flux) {
    header <- c(
      "Year1",
      "Year2",
      paste("change [", fNameNoSpace, "]", sep = ""),
      paste("slope [", fNameNoSpace, "/yr]", sep = ""),
      "change[percent]",
      "slope [percent/yr]"
    )
  } else {
  ...
  for (iFirst in 1:numPointsMinusOne) {
    ...
    for (iLast in iFirstPlusOne:numPoints) {
      ...
      results[1] <- if (is.na(yDif)) {
        ...
        format(yDif, digits = 2, width = widthLength)
      }
      results[2] <- if (is.na(yDif)) {
        blankHolder
      } else {
        format(yDif / xDif, digits = 2, width = widthLength)
      }
      results[3] <- if (is.na(yDif)) {
        blankHolder
      } else {
        format(100 * yDif / yFirst, digits = 2, width = widthLength)
      }
      results[4] <- if (is.na(yDif)) {
        blankHolder
      } else {
        format(100 * yDif / yFirst / xDif, digits = 2, width = widthLength)
      }
      cat("\n", yearPoints[iFirst], " to ", yearPoints[iLast], results)
      resultDF <- rbind(
        resultDF,
        c(yearPoints[iFirst], yearPoints[iLast], results)
      )
    }
  }
  cat("\n")
