...
plotSDLogQ <- function(
  ...
  if (endDec - window < startDec) {
    stop(paste(
      "Adjust 'window' argument to a value less than:",
      floor(endDec - startDec)
    ))
  }
  ...
  for (i in 1:numResults) {
    ...
    smallDaily <- localDaily[
      localDaily$DecYear >= firstDay & localDaily$DecYear <= lastDay,
    ]
