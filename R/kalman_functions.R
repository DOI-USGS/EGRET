...
#' The function returns an eList in which the estimates are appended to the existing Daily dataframe
#'
#' @rdname wrtdsK
#' @export
#' @param eList named list with at least the Daily, Sample, and INFO dataframes and surfaces matrix
#' @param rho numeric the lag one-day autocorrelation. Default is 0.9.
...
#' will randomly select one value for that date.  And if there are censored values in the
#' Sample data frame it will substitute a randomObserved value using makeAugmentedSamples
#' 
#' @param eList named list with at least the Daily, Sample, and INFO dataframes and surfaces matrix. 
#' @keywords internal
#' @return eList with duplicated dates in the Sample data frame randomly sampled and randomObserved values are appended.
#' @examples 
#' eList <- Choptank_eList
#' 
#' eList <- cleanUp(eList)
...
#' a specified column. Used in conjunction with cleanUp to randomly select a single sample value
#' for each day when there is more than one sample on a given day. 
#' 
#' @keywords internal
...
#' populateDailySamp
#' 
#' Creates a new Daily data frame that has appended to it the trueFlux for the day and the standardized residual
#' For the day on all sampled days.
#' @keywords internal
#' @param eList named list with at least the Daily, Sample, and INFO dataframes and surfaces matrix. 
...
#' genmissing
#' 
#' Generates a single realization of an autoregression lag-one time series through a data gap
#' Conditioned on a fixed value just prior to the gap (X1) and a fixed value just after the gap (XN)
#' 
#' Written by Tim Cohn
#' 
...
#' 
#' 
#' @param eList named list with at least the Daily, Sample, and INFO dataframes and the surfaces matrix. This
#' eList must be run through \code{WRTDSKalman}.
#' @param sideBySide logical. If \code{TRUE}, the two plots will be plotted
#' side by side in a single plot, otherwise, one plot per page.
#' 
#' @export
#' @examples 
#' 
...
plotWRTDSKalman <- function(eList, sideBySide = FALSE) {
  ...
  xlim <- c(xMin,xMax)
  if(sideBySide){
    title1 <- "WRTDS in red, WRTDS-K in green"
  ...
  if(sideBySide){
    mtext(mainTitle, line = 0, side = 3, outer = TRUE, cex= 1)
  }
}
...
#' plotTimeSlice
#' 
#' Plot of either concentration or flux over time showing both
#' the WRTDS and WRTDSKalman estimates as well as the observed values.
#' Best used to look at periods of only one or two years rather than the whole record.
#' 
#' 
#' 
#' @export
#' @param eList named list with at least the Daily, Sample, and INFO dataframes and surfaces matrix. This
#' eList must be run through \code{WRTDSKalman}.
