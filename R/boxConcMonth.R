...
boxConcMonth <- function(
  ...
  plotTitle <- if (printTitle) {
    paste(
      ...
      "\nBoxplots of sample values by month"
    )
  } else {
    ""
  }
  ...
  if (tinyPlot) {
    if (!customPar) {
      par(mar = c(4, 5, 1, 0.1), cex.lab = cex.axis, tcl = 0.5)
    }
    names <- monthInfo@monthSingle
  } else {
    if (!customPar) {
      par(mar = c(5, 6, 4, 2) + 0.1, cex.lab = cex.axis, tcl = 0.5)
    }
