...
\end{alltt}
\begin{verbatim}
There are 49578 data points, and 49579 days.
...
\end{alltt}
\begin{verbatim}
There are 24169 data points, and 24169 days.
\end{verbatim}
\begin{alltt}
...
If you wish to change the period of analysis after completing modelEstimation, there is one more step.  For example, if we want to consider a 3 month season that runs from December through Feburary, we would set paStart = 12 and paLong = 3.  (Note that the period of analysis and the arguments paStart and paLong are introduced at the begining of section \ref{sec:flowHistory} of this document.) After running modelEstimation we would then run the command:
...
\hlstd{INFO} \hlkwb{<-} \hlkwd{setPA}\hlstd{()}
\hlstd{AnnualResults}\hlkwb{<-}\hlkwd{setupYears}\hlstd{()}
