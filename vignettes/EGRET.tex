...
\end{alltt}
\begin{verbatim}
There are 49505 data points, and 49506 days.

 discharge data jumps from 2013-09-18 to 2013-09-20
\end{verbatim}
\begin{alltt}
...
\end{alltt}
\begin{verbatim}
There are 35910 data points, and 35910 days.
\end{verbatim}
\begin{alltt}
...
\end{alltt}
\begin{verbatim}
There are 49612 data points, and 49612 days.
\end{verbatim}
\begin{alltt}
\hlstd{INFO} \hlkwb{<-} \hlkwd{getMetaData}\hlstd{(sta,}\hlstr{""}\hlstd{,}\hlkwc{interactive}\hlstd{=}\hlnum{FALSE}\hlstd{)}
...
\includegraphics[width=.5\linewidth,height=.5\linewidth]{figure/plotFluxQ} \caption[Flux discharge]{Flux discharge\label{fig:plotFluxQ}}
\end{figure}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{AnnualResults}\hlkwb{<-}\hlkwd{setupYears}\hlstd{(}\hlkwc{paStart}\hlstd{=}\hlnum{12}\hlstd{,}\hlkwc{paLong}\hlstd{=}\hlnum{3}\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{savePath} \hlkwb{<-} \hlstr{"C:/Users/ldecicco/WRTDS_Output/"}
...
Figures \ref{fig:plotConcQSmooth} and \ref{fig:plotConcTimeSmooth} contain legends. The placement of the legend is controlled by legendLeft and legendTop. If both are set to 0, the legend is placed in the lower left corner. Otherwise, the value specified for legendLeft places the left edge of the legend, and legendTop specifies the top edge of the legend. The units for these values are those of the plot. The legend can also be turned off with printLegend=FALSE.
