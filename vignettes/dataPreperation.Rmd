...
The exact date when the NWIS water-quality services will be shut down is not known, but it is expected to happen in the second half of the 2023. When the NWIS web service shuts down, we will be removing the `readNWISSample` function from `EGRET`. 
...
```{r convertSample}
processed_qw <- processQWData(total_nitrogen_water, FALSE)
```
...
```{r convert2}
compressedData <- compressData(processed_qw, verbose = FALSE)
