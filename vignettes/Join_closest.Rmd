...
join_qw_uv <- function(qw_data, uv_flow_qw,
  ...
  qw_less <- qw_data |> 
    ...
    mutate(qw_val_close_less = if_else(delta_time_less <= -hour_threshold, NA, {{ qw_val_uv }}),
           qw_rmk_close_less = if_else(delta_time_less <= -hour_threshold, NA, {{ qw_rmk_uv }}),
           flow_val_close_less = if_else(delta_time_less <= -hour_threshold, NA, {{ flow_val }}),
           flow_rmk_close_less = if_else(delta_time_less <= -hour_threshold, NA, {{ flow_rmk }})) |> 
  ...
  qw_greater <- qw_data |> 
    ...
    mutate(qw_val_close_greater = if_else(delta_time_greater >= hour_threshold, NA, {{ qw_val_uv }}),
           qw_rmk_close_greater = if_else(delta_time_greater >= hour_threshold, NA, {{ qw_rmk_uv }}),
           flow_val_close_greater = if_else(delta_time_greater >= hour_threshold, NA, {{ flow_val }}),
           flow_rmk_close_greater = if_else(delta_time_greater >= hour_threshold, NA, {{ flow_rmk }})) |> 
    select(- {{ qw_val_uv }}, -{{ qw_rmk_uv }},
           - {{ flow_val }}, -{{ flow_rmk }})|> 
