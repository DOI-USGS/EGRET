...
<<convertSampleData, echo=TRUE,eval=FALSE,warning=FALSE>>=
...
LoadEstCalibration$FLOW <- Daily$Q[which(Daily$Date %in% LoadEstCalibration$sample_dt)]
...
loadestModel <- loadReg(Nitrogen  ~ model(9), data = LoadEstCalib, 
                   flow = "Q", dates = "Date",flow.units="cms",
                   conc.units="mg/l",
...
BCF <- predictResp_mean/predictResp
