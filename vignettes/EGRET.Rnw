...
startDate <- min(as.character(Sample$Date)) #Gets earliest date from Sample record
# Gather discharge data:
Daily <- getDVData(siteID,"00060",startDate,endDate)
# Gather site and parameter information:
INFO<- getMetaData(siteID,parameter_cd)
# Here user must input some values for the default behavior interactive=TRUE
