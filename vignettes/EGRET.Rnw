%\VignetteIndexEntry{Introduction to the EGRETdemo package}
%\VignetteEngine{knitr::knitr}
%\VignetteDepends{}
%\VignetteSuggests{xtable, dataRetrievaldemo,extrafont}
%\VignetteImports{methods,survival, fields}
%\VignettePackage{EGRETdemo}
...
%------------------------------------------------------------
\title{Introduction to the EGRETdemo package}
%------------------------------------------------------------
\author[1]{Robert Hirsch}
\author[1]{Laura De Cicco}
\affil[1]{United States Geological Survey}

\noindent{\huge\textsf{\textbf{Introduction to the EGRETdemo package}}}
...
%------------------------------------------------------------
\section{Introduction to Exploration and Graphics for RivEr Trends (EGRETdemo)}
%------------------------------------------------------------ 

EGRETdemo includes statistics and graphics for streamflow history, water quality trends, and the modeling algorithm Weighted Regressions on Time, Discharge, and Season (WRTDS). \textbf{Please see the official EGRETdemo manual:} (\url{http://dx.doi.org/10.3133/tm4A10}) \textbf{for more information on the EGRETdemo package.} For information on getting started in R, downloading and installing the package, see section \ref{sec:appendix1}.
...
This vignette guides you through the major functions provided by the EGRETdemo package. The package dataRetrievaldemo is required for importing data in an EGRETdemo-friendly format. The dataRetrievaldemo package and installation instructions are at:
\url{https://github.com/USGS-R/dataRetrievaldemo}. Installing dataRetrievaldemo will provide a vignette similar to this document, with complete working examples of the main dataRetrievaldemo functions. This document assumes you are familiar with the dataRetrievaldemo package. Further details are in the user guide available on gitHub: \url{https://github.com/USGS-R/EGRETdemo/raw/master/inst/doc/EGRETdemo.pdf}

Any use of trade, firm, or product names is for descriptive purposes only and does not imply endorsement by the U.S. Government.

%------------------------------------------------------------
\section{EGRETdemo Workflow}
%------------------------------------------------------------ 
Subsequent sections of this vignette discuss the EGRETdemo workflow steps in greater detail. This section provides a handy cheat sheet for diving into an EGRETdemo analysis. The first example is for a flow history analysis:
...
The second workflow example is for a water quality analysis. It includes data retrieval, merging of water quality and streamflow data, running the WRTDS estimation, and various plotting functions available in the EGRETdemo package.
...
%------------------------------------------------------------ 
The Daily dataframe initially is populated with columns generated by the dataRetrievaldemo package (Table \ref{table:Daily1}).  After you run the WRTDS calculations by using the function \texttt{modelEstimation} (as will be described in section \ref{sec:wrtds}), additional columns are inserted (Table \ref{table:Daily2}).
...
%------------------------------------------------------------ 
The Sample dataframe initially is populated with columns generated by the dataRetrievaldemo package (Table \ref{table:Sample1}). After you run the WRTDS calculations using the \texttt{modelEstimation} function (as described in section \ref{sec:wrtds}), additional columns are inserted (Table \ref{table:Sample2}):
...
\end{tabular}
  ...
  \begin{tablenotes}
    \item[1] These estimates are \enquote{leave-one-out cross validation} estimates.  See the EGRETdemo Manual for more details.
  \end{tablenotes}
 \end{threeparttable}
}
\end{table}
...
%------------------------------------------------------------ 
The INFO dataframe stores information about the measurements, such as station name, parameter name, drainage area, and so forth. There can be many additional, optional columns, but the columns in Table \ref{table:Info1} are required to initiate the EGRETdemo analysis. After you run the WRTDS calculations (as described in section \ref{sec:wrtds}), additional columns (Table \ref{table:Info2}) are automatically inserted into the INFO dataframe (see the EGRETdemo Manual for complete description of each term):
...
%------------------------------------------------------------
\section{Data Retrievals Structured For Use In The EGRETdemo Package}
\label{sec:EGRETdfs}
%------------------------------------------------------------ 
EGRETdemo includes data retrieval functions that return the data in a structure that has been designed to work with the EGRETdemo R package (\url{https://github.com/USGS-R/EGRETdemo/wiki}). In general, these dataframes may be much more \enquote{R-friendly} than the raw data, and will contain additional date information that allows for efficient data analysis.

In this section, we use 3 dataRetrievaldemo functions to get sufficient data to perform an EGRETdemo analysis.  We will continue analyzing the Choptank River. The data are structured into three EGRETdemo-specific dataframes.  The daily discharge data are placed in a dataframe called Daily.  The nitrate sample data are placed in a dataframe called Sample.  The data about the site and the parameter are placed in a dataframe called INFO.  Although these dataframes were designed to work with the EGRETdemo R package, they can be very useful for a wide range of hydrology studies that don't use EGRETdemo.
...
The \texttt{readNWISInfo}, \texttt{readWQPInfo}, and \texttt{readUserInfo} functions obtain metadata, or data about the streamgage and measured parameters. Any number of columns can be included in this dataframe. Table \ref{tab:INFOtable} describes fields are required for EGRETdemo functions. 

\begin{table}[!ht]
\begin{minipage}{\linewidth}
{\footnotesize
\caption{INFO columns required in EGRETdemo functions} 
\label{tab:INFOtable}
\begin{tabular}{lll}
  \hline
...
\multicolumn{1}{c}{\textbf{\textsf{Description}}} \\  [0pt]
  \hline
  constitAbbrev & string & Constituent abbreviation, used for saving the workspace in EGRETdemo\\
...
The function \texttt{readNWISInfo} combines \texttt{getNWISSiteInfo} and \texttt{getNWISPcodeInfo}, producing one dataframe called INFO.
...
Finally, the function \texttt{readUserInfo} can be used to convert comma separated files into an INFO dataframe. 
...
%------------------------------------------------------------
The \texttt{readNWISDaily} function retrieves the daily values (discharge in this case).  It requires the inputs siteNumber, parameterCd, startDate, endDate, interactive, and convert. These arguments are described in detail in the \texttt{dataRetrievaldemo} vignette, however \texttt{"}convert\texttt{"} is a new argument (that defaults to TRUE). The convert argument tells the program to convert the values from cubic feet per second (ft\textsuperscript{3}/s) to cubic meters per second (m\textsuperscript{3}/s) as shown in the example Daily data frame in Table \ref{tab:DailyDF1}. For EGRETdemo applications with NWIS Web retrieval, do not use this argument (the default is TRUE), EGRETdemo assumes that discharge is always stored in units of cubic meters per second. If you don't want this conversion and are not using EGRETdemo, set convert=FALSE in the function call. 
...
If discharge values are negative or zero, the code will set all of these values to zero and then add a small constant to all of the daily discharge values.  This constant is 0.001 times the mean discharge.  The code will also report on the number of zero and negative values and the size of the constant.  Use EGRETdemo analysis only if the number of zero values is a very small fraction of the total days in the record (say less than 0.1\% of the days), and there are no negative discharge values.  Columns Q7 and Q30 are the 7 and 30 day running averages for the 7 or 30 days ending on this specific date. Table \ref{tab:DailyDF1} lists details of the Daily data frame.

Notice that the \enquote{Day of the year} column can span from 1 to 366. The 366 accounts for leap years. Every day has a consistent day of the year. This means, February 28\textsuperscript{th} is always the 59\textsuperscript{th} day of the year, Feb. 29\textsuperscript{th} is always the 60\textsuperscript{th} day of the year, and March 1\textsuperscript{st} is always the 61\textsuperscript{st} day of the year whether or not it is a leap year.

User-generated Sample dataframes can also be created using the \texttt{readUserDaily} function. This is discused in detail in section \ref{sec:DailyFile}.
...
%------------------------------------------------------------
The \texttt{readNWISSample} function retrieves USGS sample data from NWIS. The arguments for this function are also siteNumber, parameterCd, startDate, endDate, interactive. These are the same inputs as \texttt{getWQPqwData} or \texttt{getWQPData} as described in the previous section.
...
The \texttt{readWQPSample} function retrieves Water Quality Portal sample data (STORET, NWIS, STEWARDS). The arguments for this function are siteNumber, characteristicName, startDate, endDate, interactive. Table \ref{tab:SampleDataframe} lists details of the Sample data frame. 
...
User-generated Sample dataframes can also be created using the \texttt{readUserSample} function. This is discused in detail in section \ref{sec:SampleFile}.
...
The dataRetrievaldemo package will \enquote{add up} all the values in a given row to form the total for that sample when using the Sample dataframe. Thus, you only want to enter data that should be added together. If you want a dataframe with multiple constituents that are not summed, do not use readNWISSample, readWQPSample, or readUserSample. The raw data functions: \texttt{getWQPData}, \texttt{getNWISqwData}, \texttt{getWQPqwData}, \texttt{getWQPData} will not sum constituents, but leave them in their individual columns. 
...
For every sample, the EGRETdemo package requires a pair of numbers to define an interval in which the true value lies (ConcLow and ConcHigh). In a simple uncensored case (the reported value is above the detection limit), ConcLow equals ConcHigh and the interval collapses down to a single point. In a simple censored case, the value might be reported as \verb@<@0.2, then ConcLow=NA and ConcHigh=0.2. We use NA instead of 0 as a way to elegantly handle future logarithm calculations.
...
Section \ref{sec:userFiles} discusses inputting user-generated files. The functions \texttt{readUserSample} and \texttt{readNWISSample} assume summation with interval censoring inputs, and are discussed in sections \ref{sec:DailyFile} and \ref{sec:SampleFile}.
...
%------------------------------------------------------------
Finally, there is a function called \texttt{mergeReport} that will look at both the Daily and Sample dataframe, and populate Q and LogQ columns into the Sample dataframe. The default arguments are Daily and Sample, however if you want to use other similarly structured dataframes, you can specify localDaily or localSample. Once \texttt{mergeReport} has been run, the Sample dataframe will be augmented with the daily discharges for all the days with samples.  None of the water quality functions in EGRETdemo will work without first having run the \texttt{mergeReport} function.
...
\multicolumn{1}{c}{\textbf{\textsf{Description}}} \\ [0pt]
  ...
  [5pt]Daily\tnote{1} & \texttt{readNWISDaily} & USGS daily values \\ 
  [5pt]Daily\tnote{1} & \texttt{readUserDaily} & User-generated daily data \\ 
  ...
  [5pt]Sample\tnote{1} & \texttt{readNWISSample} & USGS water quality data\\
  [5pt]Sample\tnote{1} & \texttt{readWQPSample} & Water Quality Data Portal data \\
  [5pt]Sample\tnote{1} & \texttt{readUserSample} & User-generated sample data \\ 
  [5pt]Unit & \texttt{getNWISunitData} & Raw USGS instantaneous data \\
  [5pt]Information\tnote{1} & \texttt{readNWISInfo} & Station and parameter code information extracted from USGS\\ 
  [5pt]Information\tnote{1} & \texttt{readWQPInfo} & Station and parameter information extracted from Water Quality Portal \\
  [5pt]Information\tnote{1} & \texttt{readUserInfo} & Station and parameter information extracted from user-generated file \\ 
...
\end{tabular}
  ...
  \begin{tablenotes}
    \item[1] Indicates that the function creates a data frame suitable for use in EGRETdemo software, otherwise data is returned in the exact form that it was received.
  \end{tablenotes}
 \end{threeparttable}
}
\end{table}
...
{\footnotesize
\begin{threeparttable}[b]
\caption{dataRetrievaldemo functions organization} 
...
\hline
\textbf{NWIS:} & \texttt{getNWISSites} & \texttt{readNWISInfo}\tnote{1} & \texttt{getNWISData}  \\
\textit{Daily} & \texttt{getNWISDataAvailability}& \texttt{getNWISSiteInfo} & \texttt{readNWISDaily}\tnote{1} \\
\textit{Unit/Instantaneous} &  & \texttt{getNWISPcodeInfo} & \texttt{readNWISSample}\tnote{1} \\
\textit{Groundwater} & &  & \texttt{getNWISdvData}   \\
\textit{Water Quality} & & & \texttt{getNWISunitData}\\
 & & & \texttt{getNWISqwData} \\
\hline
\textbf{Water Quality Portal:} & \texttt{getWQPSites} & \texttt{readWQPInfo}\tnote{1} &  \texttt{readWQPSample}\tnote{1}\\
\textit{USGS} & & & \texttt{getWQPqwData} \\
\textit{EPA} & & & \texttt{getWQPData} \\
\textit{USDA} & & &  \\
\hline
\textbf{User-supplied files:} & & \texttt{readUserInfo}\tnote{1}  & \texttt{readUserDaily}\tnote{1}  \\
\textit{Daily} & & & \texttt{readUserSample}\tnote{1}  \\
\textit{Sample} & & &  \\
...
\begin{minipage}{\linewidth}
{\footnotesize
\caption{Supplemental dataRetrievaldemo functions} 
...
\multicolumn{1}{c}{\textbf{\textsf{Description}}} \\  [0pt]
  ...
  [5pt]\texttt{importRDB1} & Retrieves and converts RDB data to dataframe\\
  [5pt]\texttt{importWaterML1} & Retrieves and converts WaterML1 data to dataframe\\
  [5pt]\texttt{importWaterML2} & Retrieves and converts WaterML2 data to dataframe\\
...
%------------------------------------------------------------ 
This section describes functions included in the EGRETdemo package that provide a variety of table and graphical outputs for examining discharge statistics based on time-series smoothing. These functions are designed for studies of long-term change and work best for daily discharge data sets of 50 years or longer. This type of analysis might be useful for studying issues such as the influence of land use change, water management change, or climate change on discharge conditions.  This includes potential impacts on average discharges, high discharges, and low discharges, at annual time scales as well as seasonal or monthly time scales. 

At this point it is assumed that you can load the daily discharge record into R, create the Daily dataframe, and enter the required metadata into the INFO dataframe. If not, see the dataRetrievaldemo vignette:
...
This section shows examples of the available plots appropriate for studying discharge history once the annualSeries has been created. The plots here use the default variable input options.  For any function, you can get a complete list of input variables (as described in section \ref{sec:flowHistoryVariables}) in a help file by typing a ? before the function name in the R console. The EGRETdemo manual has more detailed information for each plot type (\href{https://github.com/USGS-R/EGRETdemo/raw/Documentation/draft+user+guide+for+EGRETdemo+and+dataRetrievaldemo+2014-04-14.pdf}{link to download}). Finally, see section \ref{app:savingPlots} for information on saving plots.
...
The other table option is \texttt{tableChange}. This is a function that provides for the computation of changes or slopes between any selected pairs of time points.  These computations are made only on the flow-normalized results. A detailed explaination of \enquote{flow-normalized} result is in the official EGRETdemo manual.
...
The basic plotting options were shown in the section \ref{sec:wrtdsResults}.  This section demonstrates some ways to extend the capabilities of the EGRETdemo plots. EGRETdemo plots use R's basic plotting options. You set many of the formatting details of plotting routines in R by using \enquote{Graphical Parameters}.  To read about all of these graphical parameters see \texttt{?par}.  When the graphical functions in EGRETdemo are coded, a set of default values for many of these parameters are chosen, but you can override all of these default values. Additionally, you can add features to a plot after calling the plot function. To change the plot margins (mar), font, or other graphical parameters initially assigned, set the argument customPar to TRUE.
...
[5pt]lty & Line type number (such as dash or dot) & integer values\\
   \hline
\end{tabular}
\caption{Useful plotting parameters to adjust in EGRETdemo plotting functions.  For details of any of these see ?par.} 
\label{table:tableChangeSingle}
}
\end{table}
...
\label{sec:appendix1}
%------------------------------------------------------------ 
This section describes the options for installing the EGRETdemo package.
...
%------------------------------------------------------------
\subsection{R User: Installing EGRETdemo}
%------------------------------------------------------------ 
To install the EGRETdemo packages and its dependencies:
