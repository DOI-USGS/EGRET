...
\label{sec:wqpINFO}
%------------------------------------------------------------ 
It is also possible to create the INFO dataframe using information from the Water Quality Portal. As with \texttt{readWQPSample}, if the requested site is a USGS siteNumber, \enquote{USGS-} needs to be appended to the siteNumber.
