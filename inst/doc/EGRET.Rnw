...
print(data.table, caption.placement="top",include.rownames=FALSE,table.placement="!h")
@
...
<<label=colNamesInfo, echo=FALSE,results=tex>>=
ColumnName <- c("shortName", "staAbbrev", "paramShortName", "constitAbbrev","drainSqKm","paStart footnote", "paLong footnote")
Type <- c("string", "string","string", "string","numeric","integer (1-12)","integer (1-12)")
Description <- c("Name of site, suitable for use in graphical headings", "Abbreviation for station name, used in saveResults","Name of constituent, suitable for use in graphical headings", "Abbreviation for constituent name, used in saveResults","Drainage area in km^2","Starting month of period of analysis", "Length of period of analysis in months")

DF <- data.frame(ColumnName,Type,Description)

data.table <- xtable(DF, caption="INFO dataframe", label="table:Info1")
print(data.table, caption.placement="top",include.rownames=FALSE,table.placement="!h"
      , sanitize.text.function=function(str)gsub("footnote","\\footnotemark[1]",str,fixed=TRUE)
      )
@
\footnotetext[1]{paStart and paLong can be inserted using the setPA function}
...
INFORed <- setPA(paStart=6, paLong=3)
...
%------------------------------------------------------------
Similar to the function plotFlowSingle, the printSeries will print the requested flow statistics (Table \ref{table:istat})
